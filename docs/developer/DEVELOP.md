...
### Automatically via a git hook

A post-checkout hook can set up new worktrees automatically. Install it once in your repository
...
This copies `dist/hooks/post-checkout` to `.git/hooks/` and makes it executable. From then on, every new worktree will automatically:
...
If the hook is not installed, create the worktree and set it up yourself:
...
Local dependencies could potentially have compatibility issues with project dependencies. You can use `uv export -o [constraint-file]` and using the file generated as a constraint file for `local-requirements.in` (e.g. `-c constraint-file`) before compiling. `make compile-deps` will export the project dependencies to `cons-requirements.txt` (can be used by `local-requirements.in`), compile, and then delete `cons-requirements.txt`.
...
Both of these are dev dependencies and allow you to record the request/response combination from tests that perform network requests. This information is then saved in "cassette" files and subsequent runs of the tests will use those cassette files instead of performing network calls.
