...
class TestSRPReportNoAutoCreation:
    ...
    @pytest.mark.enable_signals
    def test_srp_report_is_when_flaw_is_saved(self):
        """
        Test that SRP Reports are created when the flaw is saved.
        """
        # Arrange
        start_time = timezone.now()
        flaw = FlawFactory(
        ...
        flaw.timer_started_at = start_time
        flaw.save()

        # Assert
        assert SRPReport.objects.filter(flaw=flaw).count() == 1
        assert SRPReport.objects.get(flaw=flaw).timer_started_at == start_time
