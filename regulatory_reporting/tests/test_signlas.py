...
class TestSRPReportNoAutoCreation:
    ...
    def test_srp_report_acl_is_updated_when_flaw_is_saved(
        self, acl_read, acl_write, alternate_acl_read, alternate_acl_write
    ):
        """
        ...
        assert SRPReport.objects.filter(flaw=flaw).count() == 1
        srp_report = SRPReport.objects.get(flaw=flaw)
        assert srp_report.acl_read == acl_read
        assert srp_report.acl_write == acl_write

        flaw.acl_read = alternate_acl_read
        flaw.acl_write = alternate_acl_write
        flaw.save()
        ...
        assert SRPReport.objects.filter(flaw=flaw).count() == 1
        srp_report = SRPReport.objects.get(flaw=flaw)
        assert srp_report.acl_read == alternate_acl_read
        assert srp_report.acl_write == alternate_acl_write
