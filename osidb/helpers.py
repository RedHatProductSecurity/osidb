...
class JSONSocketHandler(logging.handlers.SocketHandler):
    ...
    def makeSocket(self, *args, **kwargs):
        ...
        context.load_verify_locations(
            cafile="/opt/app-root/src/etc/logstash/certs/ca.crt"
        )
        context.load_cert_chain(
            certfile="/opt/app-root/src/etc/logstash/certs/logstash.crt",
            keyfile="/opt/app-root/src/etc/logstash/certs/logstash.key",
        )
    ...
    def emit(self, record):
        ...
        If there is an error with the socket, it will retry sending.
        If all retries fail, it falls back to writing the log to a file.
        """
        try:
