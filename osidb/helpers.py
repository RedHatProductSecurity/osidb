...
def get_execution_env() -> str:
    ...
    """
    env = getenv("OSIDB_ENV", "")
    if env:
        return env
    module = getenv("DJANGO_SETTINGS_MODULE", "")
    if "deployed" in module:
        return "prod"
...
def get_jira_cloud_id(kerberos_id: str) -> str:
    ...
    try:
        ...
        users = list(connector.jira_conn.search_users(query=email))
    except Exception:
    ...
    # Mutiple results
    users = [u for u in users if u.emailAddress == email]
