...
class TestEndpointsFlawsLabels:
    ...
    def test_delete_product_label_without_write_returns_404(
...
class TestFlawLabelsV2CRUD:
    ...
    def test_list_labels(self, auth_client, test_api_v2_uri):
        ...
        label_names = {r["name"] for r in results}
        assert label_names == {"context-label", "my-alias"}

        # Verify v2 flaw response nests labels with "name" (not v1 "label")
        flaw_response = auth_client().get(f"{test_api_v2_uri}/flaws/{self.flaw.uuid}")
        assert flaw_response.status_code == status.HTTP_200_OK
        flaw_labels = flaw_response.json()["labels"]
        flaw_label_names = {fl["name"] for fl in flaw_labels}
        assert "context-label" in flaw_label_names
        assert "my-alias" in flaw_label_names
