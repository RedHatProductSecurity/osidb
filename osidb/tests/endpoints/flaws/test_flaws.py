...
class TestEndpointsFlaws:
    ...
    def test_flaw_index(self, auth_client, test_api_v2_uri):
        """
        Test that flaw index returns uuid and local_updated_dt pair
        """
        flaw1 = FlawFactory(embargoed=False)
        flaw2 = FlawFactory(embargoed=True)

        response = auth_client().get(f"{test_api_v2_uri}/flaws/index")
        assert response.status_code == 200

        body = {item[0]: item[1] for item in response.data["results"]}

        assert body[flaw1.uuid] == flaw1.local_updated_dt
        assert body[flaw2.uuid] == flaw2.local_updated_dt

    def test_flaw_cve_index(self, auth_client, test_api_v2_uri):
        """
        Test that flaw index returns cve_id and local_updated_dt pair.
        ...
        assert len(body) == 2
        assert body[flaw1.cve_id] == flaw1.local_updated_dt
        assert body[flaw2.cve_id] == flaw2.local_updated_dt
