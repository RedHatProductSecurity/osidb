...
def assertNumQueriesLessThan(max_queries,data={}, using="default"):
    """
    ...
    class _AssertNumQueriesLessThan(CaptureQueriesContext):
        def __exit__(self, exc_type, exc_value, traceback):
            ...
            queries_total_time = sum([Decimal(q["time"]) for q in self.captured_queries])
            if not embargoed:
...
class TestQuerySetRegression:
    ...
    def test_flaw_promote(
        ...
        with assertNumQueriesLessThan(expected_queries, data=dict( embargoed=embargoed, affect_quantity=affect_quantity, expected_queries=expected_queries)):
            response = auth_client().post(
                f"{test_api_uri}/flaws/{flaw.uuid}/promote",
                data={},
                format="json",
