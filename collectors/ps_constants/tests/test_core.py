...
class TestPsConstantsCollection:
    ...
    def test_sync_cveorg_keywords(self):
        ...
        assert Allowlist.objects.count() == 1
        assert AllowlistSpecialCase.objects.count() == 1
        assert Blocklist.objects.count() == 4
        assert BlocklistSpecialCase.objects.count() == 1
    ...
    def test_failed_sync_cveorg_keywords(self):
        ...
        error_msg = "The ps-constants repository does not contain the expected CVEorg keyword lists."
        with pytest.raises(KeyError, match=error_msg):
            sync_cveorg_keywords(mock_keywords)

        assert Allowlist.objects.count() == 0
        assert AllowlistSpecialCase.objects.count() == 0
        assert Blocklist.objects.count() == 0
        assert BlocklistSpecialCase.objects.count() == 0
