...
from apps.workflows import workflow as workflow_module
...
class TestReclassificationDateGuard:
    ...
    def test_old_done_flaw_not_reclassified(self, monkeypatch):
        ...
        monkeypatch.setattr(
            workflow_module, "WORKFLOW_RECLASSIFICATION_START_DATE", self.CUTOFF
        )
        flaw = FlawFactory(
            embargoed=False,
    ...
    def test_no_restriction_when_cutoff_unset(self, monkeypatch):
        ...
        monkeypatch.setattr(
            workflow_module, "WORKFLOW_RECLASSIFICATION_START_DATE", None
        )
        flaw = FlawFactory(
            embargoed=False,
            task_key="TASK-UNSET",
