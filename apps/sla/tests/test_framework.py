...
slo:
  ...
  type: calendar days
"""
            if "major_incident_state" not in flaw_kargs:
                MI = {"major_incident_state": Flaw.FlawMajorIncident.NOVALUE}
            else:
                MI = {}
            flaw = FlawFactory(embargoed=embargoed, **flaw_kargs, **MI)
            ...
            impact = {"impact": None} if "impact" not in affect_kargs else {}
            ...
            print(f"flaw.impact: {flaw.impact}")
            print(f"affect.impact: {affect.impact}")
