...
def _is_go_stdlib(components: list[str]) -> bool:
    ...
    for c in components:
        ...
        if (
            ...
            and first_seg in GO_STDLIB_PACKAGES
        ):
            return True
    return False
...
def _pre_filter_component(
    ...
    has_custom_mapping = ComponentMapEntry.objects.filter(
...
def sync_flaw_affects_from_newcli(flaw_id: str) -> dict[str, Any]:
    ...
    for flaw_component in components:
        ...
        for ecosystem in ecosystems:
            pre_filter = _pre_filter_component(flaw, flaw_component, ecosystem)
            _apply_label(flaw, pre_filter.label)

            if pre_filter.action != "search":
                # The component was pre-filtered out, e.g. by a blocklist entry,
                # so no need to query lib_newtopia
                logger.info(
            ...
            for resolved_name in pre_filter.resolved_names:
                results = _query_newtopia(
                    resolved_name, ps_modules, ecosystem=ecosystem
                )
