...
def test_hummingbird_sync_no_labels_applied(
    ...
    normally trigger manual-triage or potential-rejection in ACE.
    """
    from apps.ace.tasks import sync_hummingbird_affects
    from osidb.models.flaw.label import WorkflowLabel
...
def test_hummingbird_sync_still_applies_version_checking(
    ...
    # OSV fixed at 3.0.9, so 3.0.8 is affected, 3.0.10 is not
    openssl_results = [
        SimpleNamespace(
            ps_update_stream="hummingbird-1",
            purls=["pkg:rpm/redhat/openssl@3.0.8-1.hum1?arch=src"],
        ),
        SimpleNamespace(
            ps_update_stream="hummingbird-1",
            ...
            return_value=MagicMock(
                search=MagicMock(
                    return_value=MagicMock(
                        filter=MagicMock(
                            return_value=MagicMock(
                                all=MagicMock(return_value=openssl_results)
                            )
                        )
                    )
                )
    ...
    # 3.0.10 should be NOTAFFECTED
    notaffected = flaw.affects.filter(
        affectedness=Affect.AffectAffectedness.NOTAFFECTED
    )
    assert notaffected.count() == 1
    assert "3.0.10" in str(notaffected.first().purl)
...
def test_ace_sync_excludes_hummingbird_modules(monkeypatch, ace_enabled):
    """
    ...
    def _search(terms, **kwargs):
        ...
        def _filter(products=None, **fkwargs):
            ...
            def _exclude(products=None, **ekwargs):
                if products is not None:
                    exclude_calls.append(("exclude", products))
                excluded_qs = MagicMock()
                excluded_qs.all.return_value = []
                return excluded_qs
    ...
    # Should have called exclude(products=["hummingbird-1"])
    assert ("exclude", ["hummingbird-1"]) in exclude_calls
