...
from apps.regulatory_reporting.models.upstream import (
    UpstreamNotification,
    UpstreamProject,
)
from apps.regulatory_reporting.settings import regulatory_reporting_settings
from apps.regulatory_reporting.tasks import (
...
class TestSendEmailAction:
    @patch(
        "apps.regulatory_reporting.api_views.upstream_notifications.async_send_email"
    )
    def test_send_email_success(self, mock_task, auth_client):
        ...
        assert (
            ...
            == "apps.regulatory_reporting.tasks.mark_upstream_notification_sent"
        )
...
class TestUpstreamNotificationTasks:
    ...
    def test_mark_upstream_notification_sent_logs_success(self, caplog):
        ...
        task_logger = logging.getLogger("apps.regulatory_reporting.tasks")
        task_logger.addHandler(caplog.handler)
        try:
            with caplog.at_level(
                logging.INFO, logger="apps.regulatory_reporting.tasks"
            ):
    ...
    def test_mark_upstream_notification_failed_logs_error(self, caplog):
        ...
        try:
            with caplog.at_level(
                logging.ERROR, logger="apps.regulatory_reporting.tasks"
            ):
