...
class TestAuditTablesRLS:
    ...
    def test_additional_information_request_audit_is_append_only(self):
        AdditionalInformationRequestAudit = apps.get_model(
            "regulatory_reporting", "AdditionalInformationRequestAudit"
        )

        set_user_acls(settings.ALL_GROUPS)
        air = AdditionalInformationRequestFactory(
            milestone__srp_report__flaw__embargoed=False
        )
        air_pk = air.pk
        events = AdditionalInformationRequestAudit.objects.filter(pgh_obj=air_pk)
        assert events.filter(pgh_label="insert").count() == 1
        ...
        assert air.delete()
        assert (
            AdditionalInformationRequestAudit.objects.filter(
                pgh_obj=air_pk, pgh_label="delete"
            ).count()
            == 1
        )
