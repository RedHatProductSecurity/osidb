...
class TestTrackerSuggestionsV1:
    """
    Test Tracker Suggestion generation for the v1 API
    """
    ...
    def test_trackers_file_offer_embargoed(self, auth_client, test_app_api_v1_uri):
        """
        Test auto tracker auto filing defined in product
    ...
    def test_trackers_file_offer_unsupported(self, auth_client, test_app_api_v1_uri):
        """
    ...
    def test_trackers_file_offer_exclude_existing_trackers(
        ...
        url = f"{test_app_api_v1_uri}/file"
...
class TestTrackerSuggestions:
    """
    Test Tracker Suggestion generation for the latest API version
    """
    ...
    def test_trackers_file_offer_invalid(
        ...
        if is_valid:
            assert len(res["not_applicable"]) == 0
            assert len(res["streams_components"]) > 0
            assert (
                res["streams_components"][0]["ps_update_stream"] == "regular-stream-1"
            )
        else:
            assert len(res["not_applicable"]) == 1
            assert res["not_applicable"][0]["uuid"] == str(affect.uuid)
    ...
    def test_trackers_file_offer_embargoed(self, auth_client, test_app_api_uri):
        ...
        assert "not_applicable" in res and len(res["not_applicable"]) > 0
        assert res["not_applicable"][0]["uuid"] == str(affect_embargoed.uuid)
        assert "streams_components" in res and len(res["streams_components"]) > 0
        assert res["streams_components"][0]["ps_update_stream"] == "regular-stream-1"
    ...
    def test_trackers_file_offer_affect_states(
        ...
        if should_suggest:
            assert len(res["not_applicable"]) == 0
            assert res["streams_components"][0]["affect"]["uuid"] == str(affect.uuid)
        else:
            assert len(res["streams_components"]) == 0
            assert res["not_applicable"][0]["uuid"] == str(affect.uuid)
    ...
    def test_trackers_file_offer_unsupported(self, auth_client, test_app_api_uri):
        ...
        with freeze_time(timezone.now() + timezone.timedelta(3)):
            ...
            res = response.json()
            assert res["not_applicable"]
            assert not res["streams_components"]
    ...
    def test_trackers_file_offer(
        ...
        if not res["streams_components"]:
            ...
            selected_streams = sorted(
                stream["ps_update_stream"]
                for stream in res["streams_components"]
                if stream["offer"] and stream["offer"]["selected"]
            )
...
class TestDefaultHandler:
    ...
    def test_is_applicable_impact(self, impact, is_applicable):
        ...
        assert is_applicable == DefaultHandler.is_applicable(affect, impact, ps_module)
...
class TestUnackedHandler:
    ...
    def test_get_offer_present(self):
        ...
        offer = framework.file_tracker_offer(affect, Impact.MODERATE, ps_update_stream)
        assert offer
