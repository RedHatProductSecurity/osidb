...
class TestTrackerJiraQueryBuilder:
    ...
    def test_severity_field(
        ...
        """
        JiraProjectFields.objects.filter(field_name="Severity").delete()
        if not missing:
            if not wrong:
                JiraProjectFields(
                    project_key="FOOPROJECT",
...
"""
        ...
        # Create a fresh tracker copy to avoid state leakage to other tests
        tracker_copy = TrackerFactory(
            affects=[affect],
            embargoed=flaw.embargoed,
            ps_update_stream=ps_update_stream.name,
            type=Tracker.TrackerType.JIRA,
        )
        tracker_copy.meta_attr["labels"] = json.dumps(["nonstandard-sla"])
        query = TrackerJiraQueryBuilder(tracker_copy).query

    @pytest.mark.parametrize(
        "impact,sla_end_date_str,expected_adjustment",
        [
            # IMPORTANT/MODERATE with dates in shutdown period - should adjust
            (Impact.IMPORTANT, "2024-12-24", True),  # Dec 24
            (Impact.IMPORTANT, "2024-12-25", True),  # Dec 25
            (Impact.IMPORTANT, "2024-12-31", True),  # Dec 31
            (Impact.IMPORTANT, "2025-01-01", True),  # Jan 1
            (Impact.IMPORTANT, "2025-01-02", True),  # Jan 2
            (Impact.MODERATE, "2024-12-24", True),  # Dec 24
            (Impact.MODERATE, "2025-01-01", True),  # Jan 1
            (Impact.LOW, "2024-12-25", True),  # Dec 25
            # CRITICAL with dates in shutdown period - should NOT adjust
            (Impact.CRITICAL, "2024-12-25", False),  # Dec 25
            (Impact.CRITICAL, "2025-01-01", False),  # Jan 1
            # IMPORTANT/MODERATE with dates outside shutdown period - should NOT adjust
            (Impact.IMPORTANT, "2024-12-23", False),  # Dec 23 (before)
            (Impact.IMPORTANT, "2025-01-03", False),  # Jan 3 (after)
            # other dates - should NOT adjust
            (Impact.MODERATE, "2024-06-15", False),  # weekend not in shutdown period
        ],
    )
    def test_adjust_sla_date_for_shutdown(
        self, impact, sla_end_date_str, expected_adjustment
    ):
        """
        Test that adjust_sla_date_for_shutdown correctly adjusts dates for IMPORTANT/MODERATE
        CVEs when they fall in the shutdown period (Dec 24 - Jan 2).
        """
        flaw = FlawFactory(embargoed=False, impact=impact)
        ps_module = PsModuleFactory(
            bts_key="RHEL",
            bts_name="jboss",
            private_trackers_allowed=False,
        ...
        affect = AffectFactory(
            ...
            resolution=Affect.AffectResolution.DELEGATED,
            ps_update_stream=ps_update_stream.name,
            impact="",  # Use flaw's impact instead of random affect impact
        )
        tracker = TrackerFactory(
            affects=[affect],
            embargoed=flaw.embargoed,
        ...
        sla_end_date = make_aware(datetime.strptime(sla_end_date_str, "%Y-%m-%d"))
        query_builder = TrackerJiraQueryBuilder(tracker)
        adjusted_date = query_builder.adjust_sla_date_for_shutdown(sla_end_date)

        assert flaw.impact == impact
        assert tracker.aggregated_impact == impact
        if expected_adjustment:
            # Should be adjusted to Jan 8 (or next business day if Jan 8 is not a business day)
            assert adjusted_date.month == 1
            assert adjusted_date.day >= 8
            if sla_end_date.month == 12:
                assert adjusted_date.year == sla_end_date.year + 1
            else:
                assert adjusted_date.year == sla_end_date.year

            # Preserve time and timezone
            assert adjusted_date.hour == sla_end_date.hour
            assert adjusted_date.minute == sla_end_date.minute
            if sla_end_date.tzinfo:
                assert adjusted_date.tzinfo == sla_end_date.tzinfo
        else:
            # Should remain unchanged
            assert adjusted_date == sla_end_date

    def test_adjust_sla_date_for_shutdown_jan8_sunday(
        self, clean_policies, setup_sample_external_resources
    ):
        """
        Test that when Jan 8 falls on a Sunday, the date is adjusted to the next business day.
        Jan 8, 2023 was a Sunday, so it should adjust to Jan 9 (Monday).
        """
        ...
        # Ensure tracker has no labels from previous tests that might leak state
        if "labels" in tracker.meta_attr:
            del tracker.meta_attr["labels"]
            tracker.save()

        # Jan 8, 2023 was a Sunday
        sla_end_date = make_aware(
            datetime(2023, 1, 1)
        )  # Jan 1, 2023 (in shutdown period)
        query_builder = TrackerJiraQueryBuilder(tracker)
        adjusted_date = query_builder.adjust_sla_date_for_shutdown(sla_end_date)

        # make sure the impact is still IMPORTANT
        assert flaw.impact == Impact.IMPORTANT
        assert tracker.aggregated_impact == Impact.IMPORTANT

        # Should adjust to Jan 9 (Monday) since Jan 8, 2023 was Sunday
        assert adjusted_date.month == 1
        assert adjusted_date.day == 9
        assert adjusted_date.year == 2023

    def test_generate_sla_shutdown_adjustment_important(
        self, clean_policies, setup_sample_external_resources
    ):
        """
        Test that generate_sla adjusts IMPORTANT tracker SLA dates that fall in shutdown period.
        """
        # Create IMPORTANT tracker with SLA that falls in shutdown period
        flaw = FlawFactory(
            embargoed=False,
            impact=Impact.IMPORTANT,
            reported_dt=make_aware(datetime(2024, 12, 20)),  # Dec 20
            source="REDHAT",
        )
        ps_module = PsModuleFactory(
            bts_key="RHEL",
        ...
        # SLA policy: 5 calendar days from reported date = Dec 25 (in shutdown period)
        sla_file = """
---
name: Important Shutdown Test
description: Test SLA adjustment for shutdown period
conditions:
  flaw:
    - is not embargoed
  affect:
...
"""
        ...
        assert "customfield_12326740" in query["fields"]
        sla_date_str = query["fields"]["customfield_12326740"]
        # Should be adjusted to Jan 8 (or next business day)
        # Format: "YYYY-MM-DDTHH:MM:SS.mmm+0000"
        assert sla_date_str.startswith("2025-01-0")  # Jan 8 or later
        assert int(sla_date_str[8:10]) >= 8  # Day >= 8

    def test_generate_sla_shutdown_adjustment_moderate(
        self, clean_policies, setup_sample_external_resources
    ):
        """
        Test that generate_sla adjusts MODERATE tracker SLA dates that fall in shutdown period.
        """
        # Ensure we start with a clean tracker (no labels from previous tests)
        flaw = FlawFactory(
            embargoed=False,
            impact=Impact.MODERATE,
            reported_dt=make_aware(datetime(2024, 12, 28)),  # Dec 28
            source="REDHAT",
        )
        ps_module = PsModuleFactory(
            bts_key="RHEL",
        ...
        # SLA policy: 5 calendar days from reported date = Jan 2 (in shutdown period)
        sla_file = """
---
name: Moderate Shutdown Test
description: Test SLA adjustment for shutdown period
conditions:
  flaw:
    - is not embargoed
...
"""
        ...
        sla_date_str = query["fields"]["customfield_12326740"]
        # Should be adjusted to Jan 8 (or next business day)
        assert sla_date_str.startswith("2025-01-0")
        assert int(sla_date_str[8:10]) >= 8

    def test_generate_sla_shutdown_no_adjustment_critical(
        self, clean_policies, setup_sample_external_resources
    ):
        """
        Test that generate_sla does NOT adjust CRITICAL tracker SLA dates even if in shutdown period.
        """
        flaw = FlawFactory(
            embargoed=False,
            impact=Impact.CRITICAL,
        ...
        sla_file = """
---
name: Critical Shutdown Test
description: Test that CRITICAL is NOT adjusted
conditions:
  flaw:
    - is not embargoed
  affect:
...
"""
        ...
        assert "customfield_12326740" in query["fields"]
        sla_date_str = query["fields"]["customfield_12326740"]
        # Should NOT be adjusted - should remain Dec 25
        assert sla_date_str.startswith("2024-12-25")
