...
class OldTrackerJiraQueryBuilder(TrackerQueryBuilder):  # TODO?
    """
...
# TODO mark all necessary changes
class TrackerJiraQueryBuilder(TrackerQueryBuilder):
    ...
    def generate(self):
        ...
        self.generate_base()
        self.generate_component()
        # TODO delete: self.generate_priority()
        self.generate_description()
        self.generate_labels()
        self.generate_sla()
        self.generate_summary()
        ...
        #      - examples:
        #          - Affects Version/s
        #             - generate_versions in this file
        #          - Component/s (e.g. "security")
        #             - generate_component in this file
        #          - Type
        #             - generate_base in this file
        #          - Priority
        #             - generate_priority in this file
        #          - Security Level
        #             - generate_security in this file
        #          - Labels
        #             - generate_labels in this file
        #          -
        #
        #
        ...
        if severity not in allowed_values:
            ...
            raise NoSeverityAvailableError(
                f"Jira project {self.ps_module.bts_key} does not have the severity level "
                f"{severity}; allowed Jira priority values are: {', '.join(allowed_values)}"
            )
        ...
        # with open("/tmp/q", "a") as tmpq:
        #     from pprint import pprint

        #     tmpq.write("-----\n")
        #     tmpq.write(str(pprint(self._query)))
        #     tmpq.write("\n")
        #     tmpq.write("----------\n")
    ...
    def generate_base(self):
        self._query = {
            "fields": {
                "issuetype": {"name": "Bug"},  # TODO Vulnerability type
                "project": {"key": self.ps_module.bts_key},  # TODO TPFUN for testing
            }
        }
        if self.tracker.external_system_id:
            self._query["key"] = self.tracker.external_system_id

    # TODO: waiting for https://docs.google.com/document/d/11u7Zkc4svSt-4HuimbXaB8-_nFjclVG1EauaqUjCTjs/edit?disco=AAABQboqO44
    def generate_component(self):
        """
        Generate Jira "components" field (with 1 component)
        """
